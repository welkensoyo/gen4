...
# Local imports
from dbisamtojson.columns import get_col_names
from dbisamtojson.rows import get_row_data
...
def main(fname, j=True, c=False, out='', stream=False):
    form = 'json'
    outfile = False
    # READ args
    if(j and c):
    ...
    with open(fname, 'rb') as file:
        ...
        for i in range(0, len(offsets)-1):  # len(offsets)-1
            dataset.append(get_row_data(file, cols, offsets[i], stream))
